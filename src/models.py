...
def fitFromHisto(X, y, algo):
    """Entraîne l'algorithme sur les données."""
    model = algo
    model.fit(X, y)
    return model
...
def error_real(model, X, y):
    """Calcule l'erreur réelle via cross-validation."""
...
def run_grid_search(
    ...
    if param_grid is None:
        ...
        if algo_name is None:
            raise ValueError(
                f"Impossible de détecter la grille pour {type(algo).__name__}. "
                f"Fournir param_grid manuellement."
            )
        if algo_name not in DEFAULT_PARAM_GRIDS:
            raise ValueError(
                f"Pas de grille par défaut pour '{algo_name}'. "
